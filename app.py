...
if st.button("Рекомендация от помощник"):
    ...
    # Создаём промпт
    prompt = f"""
Дай  короткий совет по покупке квартиры в городе Астане с параметрами:
...
тип жилье {house_type} здесь не пиши фолс и тру указвай как в залоге и не залоге,
в {in_pledge} залоге,
Напиши 5–6 предложения.
...
Пожелай ему  что-то хорошее, чтобы он купил квартиру, чтобы он хороший вариант и так далее 1-2 предложение
Напиши в конце что ты рад что помог пользователю, и что-то хорошее, чтобы он купил квартиру, чтобы он хороший вариант и так далее 1-2 предложение, но если хочешь обсудить подробнее это сделай ссылку в чатждпити с этим промтом
"""
